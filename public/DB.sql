...
alter table tierras add constraint fk_tierras_persona_9 foreign key (persona_id) references persona (id) on delete restrict on update restrict;
create index ix_tierras_persona_9 on tierras (persona_id);
alter table transporte add constraint fk_transporte_hijos_10 foreign key (hijos_id) references hijos (id) on delete restrict on update restrict;
create index ix_transporte_hijos_10 on transporte (hijos_id);
alter table usuarios add constraint fk_usuarios_persona_11 foreign key (persona_id) references persona (id) on delete restrict on update restrict;
create index ix_usuarios_persona_11 on usuarios (persona_id);
alter table vivienda add constraint fk_vivienda_persona_12 foreign key (persona_id) references persona (id) on delete restrict on update restrict;
create index ix_vivienda_persona_12 on vivienda (persona_id);
